package com.msaitodev.mhsw.humanmed.ui
...
internal fun AppNavHost(
    ...
    LaunchedEffect(quizResultForProcessing) {
        ...
        val questionsJson = URLEncoder.encode(
            Json.Default.encodeToString(result.questions),
            StandardCharsets.UTF_8.toString()
        )
        val answersJson = URLEncoder.encode(
            Json.Default.encodeToString(result.answers),
            StandardCharsets.UTF_8.toString()
        )

        navController.navigate(
            ResultDestination.build(
                result.score,
                result.total,
                result.isReview,
                questionsJson,
