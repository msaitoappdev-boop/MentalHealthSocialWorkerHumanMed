...
本ドキュメントは、`MentalHealthSocialWorkerHumanMed` プロジェクトをテンプレートとして、新しい資格試験アプリを立ち上げるための標準的な手順を定義します。
...
1.  **新規フォルダの作成**:
    - `C:/Users/msait/AndroidStudioProjects/` 配下に、新しいアプリ用のフォルダを作成します。（例： `another-exam-app`）

2.  **テンプレートファイルのコピー**:
    - `mentalhealth-socialworker-humanmed-app` プロジェクトから、以下のファイルとフォルダをすべて新しいプロジェクトフォルダへコピーします。
      - `app/` （`build/` フォルダは除く）
      - `gradle/`
      - `.github/`
      - `master_data/`
...
1.  **Application ID**: `app/build.gradle.kts`
    - `applicationId` を新しいものに変更。（例： `com.msaitodev.another.exam`）

2.  **アプリ名**: `app/src/main/res/values/strings.xml`
    - `app_name` を新しいアプリ名に変更。

3.  **プロジェクト名**: `settings.gradle.kts`
    - `rootProject.name` を新しいプロジェクト名に変更（例： `"AnotherExamHumanMed"`）。
...
1.  **科目（カテゴリ）定義**:
    ...
    - `app/src/main/java/com/msaitodev/mhsw/humanmed/notifications/QuizCategoryNameProvider.kt` の `when` 式を、新しい科目定義に合わせて修正。
